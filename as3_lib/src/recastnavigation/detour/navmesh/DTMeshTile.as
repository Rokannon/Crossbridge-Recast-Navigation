package recastnavigation.detour.navmesh
{
    ...
    public class DTMeshTile extends RNBase
    {
        ...
        {
            return CModule.read32(ptr + OFFSET_SALT);
        }

        public function set salt(value:int):void
        {
            CModule.write32(ptr + OFFSET_SALT, value);
        }

        /** Index to the next free link. */
        public function get linksFreeList():int
        {
            return CModule.read32(ptr + OFFSET_LINKS_FREE_LIST);
        }

        public function set linksFreeList(value:int):void
        {
            CModule.write32(ptr + OFFSET_LINKS_FREE_LIST, value);
        }
        ...
        {
            copyBytes(value.ptr, CModule.read32(ptr + OFFSET_DETAIL_MESHES) + DTPolyDetail.SIZE * index,
                      DTPolyDetail.SIZE);
        }
        ...
        {
            copyBytes(value.ptr, CModule.read32(ptr + OFFSET_OFF_MESH_CONS) + DTOffMeshConnection.SIZE * index,
                      DTOffMeshConnection.SIZE);
        }

        /** The tile data. (Not directly accessed under normal situations.) Getter. */
        public function getData(index:int):int
        {
            return CModule.read8(CModule.read32(ptr + OFFSET_DATA) + 1 * index);
        }

        /** The tile data. (Not directly accessed under normal situations.) Setter. */
        public function setData(index:int, value:int):void
        {
            CModule.write8(CModule.read32(ptr + OFFSET_DATA) + 1 * index, value);
        }

        /** Size of the tile data. */
        public function get dataSize():int
        {
            return CModule.read32(ptr + OFFSET_DATA_SIZE);
        }

        public function set dataSize(value:int):void
        {
            CModule.write32(ptr + OFFSET_DATA_SIZE, value);
        }

        /** Tile flags. (See: #dtTileFlags) */
        public function get flags():int
        {
            return CModule.read32(ptr + OFFSET_FLAGS);
        }

        public function set flags(value:int):void
        {
            CModule.write32(ptr + OFFSET_FLAGS, value);
        }
